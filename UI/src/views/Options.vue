<script setup lang="ts">
	...
	const config = reactive({
		...
		bootFaceRecog: optionsStore.getOptionValueByKey('bootFaceRecog') ? (optionsStore.getOptionValueByKey('bootFaceRecog') == 'false' ? false : true) : false,
	})
	...
	// 开机面容识别切换
	const handleBootFaceRecogChange = ()=>{
		const nextValue = config.bootFaceRecog;
		invoke("write_to_registry", {items: [
			{ key: "BOOT_FACE_RECOG", value: nextValue ? "1" : "0" }
		]}).then(()=>{
			return optionsStore.saveOptions({
				bootFaceRecog: nextValue
			});
		}).then((errorArray)=>{
			...
			config.bootFaceRecog = !nextValue;
			ElMessage.error(formatObjectString("设置开机面容识别失败: ", error));
		})
	}
	...
	const saveAppConfig = () => {
		optionsStore.saveOptions({
			...
			bootFaceRecog: config.bootFaceRecog,
		}).then((errorArray)=>{
			if(errorArray.length > 0){
				ElMessage.warning({
                    dangerouslyUseHTMLString: true,
						...
						<el-col :span="14">
							...
							<div class="option-row">
								<div class="row-text">
									...
									<p class="sub">登录界面启用面容识别（需要管理员权限）</p>
								</div>
								<el-switch v-model="config.bootFaceRecog" @change="handleBootFaceRecogChange"/>
							</div>
								<div class="option-row">
									<div class="row-text">
