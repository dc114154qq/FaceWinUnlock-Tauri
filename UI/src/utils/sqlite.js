...
async function connect() {
    ...
    try {
        ...
        let exeDir = localStorage.getItem("exe_dir");
        if (!exeDir || exeDir === "null") {
            try {
                const res = await invoke('get_exe_dir');
                if (res && res.data && res.data.path) {
                    exeDir = res.data.path;
                }
            } catch (error) {
        ...
        if (!exeDir || exeDir === "null") {
            exeDir = await executableDir();
        }
        localStorage.setItem("exe_dir", exeDir);

        // 建立连接
        db = await Database.load(`sqlite:${exeDir}\\database.db`);
