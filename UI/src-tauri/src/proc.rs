...
use crate::{
    ...
    utils::{
        api::{open_camera, stop_camera, unlock},
        pipe::{read, Client, Server},
    },
    APP_STATE, CAMERA_INDEX, DB_POOL, IS_BREAK_THREAD, IS_LOCKED, IS_RUN, MATCH_FAIL_COUNT,
    RETRY_DELAY, ROOT_DIR, TIMER_ID_LOCK_CHECK,
};
...
struct BootSettings {
    face_recog_type: String,
    face_recog_delay_ms: u64,
    retry_delay_ms: i32,
    camera_index: i32,
}
...
fn parse_delay_ms(value: String, default_secs: f32, label: &str) -> i32 {
    match value.parse::<f32>() {
        Ok(seconds) => (seconds * 1000.0) as i32,
        Err(e) => {
            error!(
                "{}秒数字符串转换失败: {}，使用默认值 {} 毫秒",
                label,
                e,
                (default_secs * 1000.0) as i32
            );
            (default_secs * 1000.0) as i32
        }
    }
...
fn load_boot_settings() -> Result<Option<BootSettings>, String> {
    let pool_guard = DB_POOL
        .lock()
        .map_err(|_| String::from("从全局变量获取连接池失败"))?;
    ...
    let is_initialized: String = conn
        .query_row(
    ...
    let face_recog_type: String = conn
        .query_row(
    ...
    let camera_index_val: String = conn
        .query_row("SELECT val FROM options WHERE key = 'camera';", [], |row| {
            row.get::<&str, String>("val")
        })
        .unwrap_or(String::from("0"));
    let camera_index = camera_index_val.parse().unwrap_or(0);

    let retry_delay_val: String = conn
        .query_row(
    ...
    let retry_delay_ms = parse_delay_ms(retry_delay_val, 10.0, "重试");

    let face_recog_delay_val: String = conn
        .query_row(
        ...
        .unwrap_or(String::from("10.0"));
    let face_recog_delay_ms = parse_delay_ms(face_recog_delay_val, 10.0, "延迟") as u64;

    Ok(Some(BootSettings {
        face_recog_type,
        face_recog_delay_ms,
        retry_delay_ms,
        camera_index,
...
pub fn start_boot_unlock_listener() {
    let settings = match load_boot_settings() {
        Ok(Some(val)) => val,
        Ok(None) => {
            info!("开机面容识别条件未满足，跳过启动");
            return;
        }
        Err(e) => {
            error!("开机面容识别配置读取失败: {}", e);
            return;
        }
    };

    CAMERA_INDEX.store(settings.camera_index, Ordering::SeqCst);
    RETRY_DELAY.store(settings.retry_delay_ms, Ordering::SeqCst);
    MATCH_FAIL_COUNT.store(0, Ordering::SeqCst);

    let kickoff_delay_ms = if settings.face_recog_type == "delay" {
        settings.face_recog_delay_ms
    } else {
        1000
    };

    std::thread::spawn(move || {
        sleep(Duration::from_millis(kickoff_delay_ms));
        if !IS_RUN.load(Ordering::SeqCst) && MATCH_FAIL_COUNT.load(Ordering::SeqCst) < MAX_RETRY {
            if let Ok(true) = run_before() {
                std::process::exit(0);
            }
            if MATCH_FAIL_COUNT.load(Ordering::SeqCst) >= MAX_RETRY {
                std::process::exit(0);
            }
        }
    });
            ...
            match read(server.handle) {
                Ok(content) => {
                    if content.contains("run")
                        && !IS_RUN.load(Ordering::SeqCst)
                        && MATCH_FAIL_COUNT.load(Ordering::SeqCst) < MAX_RETRY
                    {
                        if can_retry() {
                            info!("运行面容识别代码(开机)");
                            if let Ok(true) = run_before() {
                                        ...
                                        if is_initialized == "true" {
                                            ...
                                            if !face_recog_type.is_empty() {
                                                ...
                                                if face_recog_type == "operation" {
                                                    ...
                                                    RETRY_DELAY
                                                        .store(time_ms as i32, Ordering::SeqCst);
                                                    // 锁屏连接管道，等待管道传来的运行
                                                    ...
                                                    std::thread::spawn(move || {
                                                        let mut server = Server::new(
                                                            HSTRING::from(
                                                                r"\\.\pipe\MansonWindowsUnlockRustClient",
                                                            ),
                                                        );
                                                        let f_connected = server.connect();
                                                        // 连接失败后退出循环
...
fn run_before() -> Result<bool, String> {
    // 先打开摄像头
