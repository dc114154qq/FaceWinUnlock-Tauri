...
pub fn deploy_core_components() -> Result<CustomResult, CustomResult> {
    ...
    let mut resource_path = ROOT_DIR.join("resources").join(dll_name);

    // 检查资源文件是否存在
    if !resource_path.exists() {
        let dev_resource_path = ROOT_DIR
            .join("..")
            .join("..")
            .join("resources")
            .join(dll_name);
        if dev_resource_path.exists() {
            resource_path = dev_resource_path;
        } else {
            let dev_dll_path = ROOT_DIR
                .join("..")
                .join("..")
                .join("..")
                .join("..")
                ...
                .join("debug")
                .join("winlogon.dll");
            if dev_dll_path.exists() {
                resource_path = dev_dll_path;
            } else {
                return Err(CustomResult::error(
    ...
    let exe_path = std::env::current_exe()
        .map_err(|e| CustomResult::error(Some(format!("获取程序路径失败: {}", e)), None))?;
    ...
    {
        write_to_registry(vec![RegistryItem {
            key: String::from("APP_EXE_PATH"),
            value: exe_path_str.to_string(),
        }])?;
    }
