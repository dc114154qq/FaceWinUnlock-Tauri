...
use proc::{start_boot_unlock_listener, wnd_proc_subclass};
use tauri_plugin_log::{log::error, Target, TargetKind};
use utils::api::{
    check_global_autostart, close_app, disable_global_autostart, enable_global_autostart,
    get_camera, get_exe_dir, get_now_username, init_model, open_camera, open_directory,
    stop_camera, test_win_logon,
};
mod tray;
...
pub fn run() {
    ...
    let is_boot_unlock = args.iter().any(|arg| arg == "--boot-unlock");

    if is_boot_unlock {
        if let Ok(_) = std::fs::create_dir_all(&log_path) {
            #[cfg(windows)]
            {
                let data_dir = ROOT_DIR.join("EBWebView");
        ...
        match init_model() {
            Ok(_) => start_boot_unlock_listener(),
            Err(e) => error!("开机面容识别初始化失败: {}", e.msg),
        }
