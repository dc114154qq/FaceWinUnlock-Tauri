...
use crate::{
    utils::custom_result::CustomResult, OpenCVResource, APP_STATE, DB_POOL, GLOBAL_TRAY, ROOT_DIR,
};
use opencv::{
...
#[tauri::command]
pub fn get_exe_dir() -> Result<CustomResult, CustomResult> {
    let path = ROOT_DIR
        .to_str()
        .ok_or_else(|| CustomResult::error(Some(String::from("程序路径解析失败")), None))?;
    Ok(CustomResult::success(None, Some(json!({ "path": path }))))
}
...
pub fn init_model() -> Result<CustomResult, CustomResult> {
    ensure_resource_path_in_path();
    // 加载模型
    let mut app_state = APP_STATE
        .lock()
        .map_err(|e| CustomResult::error(Some(format!("获取app状态 {}", e)), None))?;
    if app_state.detector.is_none() {
        let resource_path = resolve_resource_path("face_detection_yunet_2023mar.onnx");
    ...
    if app_state.recognizer.is_none() {
        let resource_path = resolve_resource_path("face_recognition_sface_2021dec.onnx");
...
fn resolve_resource_path(file_name: &str) -> std::path::PathBuf {
    let primary = ROOT_DIR.join("resources").join(file_name);
    if primary.exists() {
        return primary;
    }
...
fn ensure_resource_path_in_path() {
    let resource_dir = {
        let primary = ROOT_DIR.join("resources");
        if primary.exists() {
            primary
        } else {
            ROOT_DIR.join("..").join("..").join("resources")
        }
    };

    let resource_path = match resource_dir.to_str() {
    ...
    let exists = current_path
        .split(';')
        .any(|item| item.eq_ignore_ascii_case(&resource_path));
    if !exists {
        let updated = format!("{};{}", resource_path, current_path);
...
pub fn enable_global_autostart() -> Result<CustomResult, CustomResult> {
    ...
    if let Ok(app_key) = hklm.open_subkey("SOFTWARE\\facewinunlock-tauri") {
        if let Ok(app_path) = app_key.get_value::<String, _>("APP_EXE_PATH") {
    ...
    if !output.status.success() {
        ...
        return Err(CustomResult::error(
            Some(format!("创建全用户计划任务失败: {}", err_msg)),
            None,
        ));
    }
...
pub fn unlock(user_name: String, password: String) -> windows::core::Result<()> {
    ...
    if let Err(e) = crate::utils::pipe::write(
        client.handle,
        format!("{}::FaceWinUnlock::{}", user_name, password),
    ) {
