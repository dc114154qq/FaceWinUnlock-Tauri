...
use crate::{
    ...
    CSampleCredential::SampleCredential, SharedCredentials,
};
...
static BOOT_HELPER_STARTED: AtomicBool = AtomicBool::new(false);

fn try_start_boot_helper() {
    if BOOT_HELPER_STARTED.swap(true, Ordering::SeqCst) {
        return;
    }

    let enable = match read_facewinunlock_registry("BOOT_FACE_RECOG") {
        Ok(val) => val,
        Err(e) => {
            warn!("读取开机面容识别开关失败: {}", e);
            BOOT_HELPER_STARTED.store(false, Ordering::SeqCst);
            return;
        }
    };
    ...
    let exe_path = match read_facewinunlock_registry("APP_EXE_PATH") {
        Ok(val) => val,
        Err(e) => {
            warn!("读取主程序路径失败: {}", e);
            BOOT_HELPER_STARTED.store(false, Ordering::SeqCst);
    ...
    cmd.arg("--boot-unlock").arg("--silent");
    cmd.creation_flags(CREATE_NO_WINDOW.0);
    match cmd.spawn() {
        Ok(_) => info!("已启动开机面容识别进程"),
        Err(e) => {
            error!("启动开机面容识别进程失败: {:?}", e);
            BOOT_HELPER_STARTED.store(false, Ordering::SeqCst);
        }
    }
}
...
impl ICredentialProvider_Impl for SampleProvider_Impl {
    ...
    fn SetUsageScenario(
        ...
        info!(
            "SampleProvider::SetUsageScenario - 设置使用场景: {:?}",
            cpus
        );
        let mut inner = self.inner.lock().unwrap();
    ...
    fn Advise(
        ...
        info!(
            "SampleProvider::Advise - 注册事件通知，上下文ID: {}",
            upadvisecontext
        );
        let mut inner = self.inner.lock().unwrap();
        ...
        if inner.usage_scenario == CPUS_LOGON {
            try_start_boot_helper();
        }
    ...
    fn GetFieldDescriptorCount(&self) -> windows_core::Result<u32> {
        ...
        info!(
            "SampleProvider::GetFieldDescriptorCount - 字段数量: {}",
            count
        );
        Ok(count)
    }
    ...
    fn GetFieldDescriptorAt(
        ...
        info!(
            "SampleProvider::GetFieldDescriptorAt - 获取字段 {} 的描述符",
            dwindex
        );
        unsafe {
            // 分配字段描述符的内存（使用CoTaskMemAlloc，系统会负责释放）
            let size = std::mem::size_of::<CREDENTIAL_PROVIDER_FIELD_DESCRIPTOR>();
            let ptr = windows::Win32::System::Com::CoTaskMemAlloc(size)
            ...
            let (ft, label) = match dwindex {
                ...
                _ => {
                    error!(
                        "SampleProvider::GetFieldDescriptorAt - 无效的字段索引: {}",
                        dwindex
                    );
    ...
    fn GetCredentialAt(&self, dwindex: u32) -> windows_core::Result<ICredentialProviderCredential> {
        info!(
            "SampleProvider::GetCredentialAt - 获取凭据，索引: {}",
            dwindex
        );
        if dwindex == 0 {
            let mut inner = self.inner.lock().unwrap();
            ...
            error!(
                "SampleProvider::GetCredentialAt - 无效的凭据索引: {}",
                dwindex
            );
...
pub fn retrieve_negotiate_auth_package() -> windows_core::Result<u32> {
    ...
    let status =
        unsafe { LsaLookupAuthenticationPackage(lsa_handle, &package_name, &mut package_id) };
